...
def some_state_handler(bot: TelegramBot, state: UserStates) -> Union[TelegramMessage, Keyboard|InlineKeyboard]:
    '''TODO: Write your state handler. For each state of the user defined in UserStates enum, you must define a handler.'''

def some_inline_query_handler(bot: TelegramBot, callback_query: TelegramCallbackQuery)-> Union[TelegramMessage, Keyboard|InlineKeyboard]:
...
# Parallel Jovbs:
def some_parallel_job(bot: TelegramBot)-> Union[TelegramMessage, Keyboard|InlineKeyboard]:
